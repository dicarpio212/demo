...
const checkClassConflict = (
  ...
  for (const existingClass of existingClasses) {
    if ("id" in existingClass && classIdsToIgnore.includes(existingClass.id))
      continue;
    if (
      "status" in existingClass &&
      (existingClass.status === ClassStatus.Batal ||
        existingClass.status === ClassStatus.Selesai)
    )
      continue;
    ...
    if (isTimeOverlapping(newStart, newEnd, existingStart, existingEnd)) {
      if (
        existingClass.location.toUpperCase() === newClass.location.toUpperCase()
      ) {
      ...
      if ("lecturers" in existingClass) {
        const isLecturerInvolved = existingClass.lecturers.some(
          (lec) => normalizeName(lec) === normalizedLecturerName
        );
...
const getUpdatedUser = (user: User, currentDate: Date): User => {
  ...
  try {
    ...
    const periodsPassed =
      (current.year - registration.year) * 2 +
      (current.period - registration.period);

    const classLetterMatch = user.classType.match(/[A-D]$/i);
    const classLetter = classLetterMatch
      ? classLetterMatch[0].toUpperCase()
      : "A";
...
const App: React.FC = () => {
  ...
  const [archivedClassIds, setArchivedClassIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const [selectedUserByAdmin, setSelectedUserByAdmin] = useState<User | null>(
    null
  );
  ...
  const [onScreenNotification, setOnScreenNotification] = useState<
    string | null
  >(null);
  const [studentDeletedClassIds, setStudentDeletedClassIds] = useState<
    Set<string>
  >(new Set());
  const [lecturerDeletedClassIds, setLecturerDeletedClassIds] = useState<
    Set<string>
  >(new Set());
  ...
  const [calendarDisplayDate, setCalendarDisplayDate] = useState<Date | null>(
    null
  );
  ...
  const fetchAppUserData = async (userId: string) => {
    try {
      ...
      const createdPrefs = (await databases.createDocument(
        DATABASE_ID,
        PREFERENCES_COLLECTION_ID,
        userId,
        newPrefs
  ...
  const fetchAllData = async () => {
    try {
      const [usersResponse, classesResponse, notificationsResponse] =
        await Promise.all([
          databases.listDocuments(DATABASE_ID, USERS_COLLECTION_ID, [
            Query.limit(5000),
          ]),
          databases.listDocuments(DATABASE_ID, CLASSES_COLLECTION_ID, [
            Query.limit(5000),
          ]),
      ...
      setAllClasses(
        classesResponse.documents.map((c: any) => ({
          ...c,
      ...
      setNotifications(
        notificationsResponse.documents.map((n: any) => ({
          ...n,
  ...
  useEffect(() => {
    if (
      !userPreferences ||
      (themeKey === userPreferences.themeKey &&
  ...
  const handleLogin = async (
    ...
    try {
      ...
      const history = (prefs.loginHistory || []).filter(
      ...
      if (user.role === "administrator") {
        ...
        user.nim_nip === "" ||
        (user.role === "student" && !user.classType)
      ) {
  ...
  const updateUserProfile = (updatedUser: User): string | null => {
    try {
      ...
      if (
        ...
        currentUser?.password_raw
      ) {
        account.updateEmail(updatedUser.username, currentUser.password_raw);
      }
      if (
        updatedUser.password_raw !== currentUser?.password_raw &&
        updatedUser.password_raw &&
        currentUser?.password_raw
      ) {
      ...
      if (currentUser?.name !== updatedUser.name) {
        ...
        setAllClasses((prevClasses) =>
          prevClasses.map((cls) => ({
            ...cls,
            lecturers: cls.lecturers.map((lecturer) =>
              lecturer === currentUser?.name ? updatedUser.name : lecturer
            ),
          }))
        );
      }
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    if (userToDelete.role === "lecturer") {
      const classesToDelete = allClasses.filter((cls) =>
        cls.lecturers.some((lec) => lec === userToDelete.name)
      );
      await Promise.all(
        classesToDelete.map((cls) =>
          databases.deleteDocument(DATABASE_ID, CLASSES_COLLECTION_ID, cls.$id!)
        )
      );
    }
  ...
  const markNotificationAsRead = (notificationId: string) => {
    ...
    if (notif && !notif.readBy.includes(currentUser.$id)) {
      ...
      setNotifications((prev) =>
        prev.map((n) =>
          n.id === notificationId ? { ...n, readBy: newReadBy } : n
        )
      );
    }
  };
  ...
  const deleteAllNotifications = () => {
    ...
    setNotifications((prevNotifs) =>
      prevNotifs.map((notif) => ({
        ...notif,
        deletedBy: [...notif.deletedBy, currentUser!.$id],
      }))
    );
  };

  const handleCloseDetail = () => {
    setLastView(View.CLASS_DETAIL);
  ...
  const cancelClass = async (classId: string) => {
    ...
    if (
      !baseClass ||
      baseClass.status === ClassStatus.Selesai ||
      baseClass.status === ClassStatus.Batal
    ) {
      return;
    }
    ...
    const createdNotif = await databases.createDocument(
      DATABASE_ID,
      NOTIFICATIONS_COLLECTION_ID,
      ID.unique(),
      newNotification
  ...
  const deleteClass = (classId: string) => {
    ...
    const deletedSetter =
      currentUser.role === "student"
        ? setStudentDeletedClassIds
        : setLecturerDeletedClassIds;
    ...
    if (view === View.CLASS_DETAIL && selectedClass?.id === classId)
      handleCloseDetail();
  };
  ...
  const allUserVisibleClasses = useMemo(() => {
    ...
    const suspendedLecturerNames = new Set(
      allUsers
        .filter((u) => u.role === "lecturer" && u.isSuspended)
        .map((u) => u.name)
    );
    const deletedIds =
      currentUser.role === "student"
    ...
    if (currentUser.role === "student" && currentUser.registrationDate) {
      ...
      notifications.forEach((n) => {
        if (n.message.includes("telah dibatalkan")) {
          ...
          if (!existingTime || notificationTime < existingTime)
            cancellationTimes.set(n.classId, notificationTime);
        }
      });
    ...
    if (currentUser.role === "lecturer") {
      ...
      classes = classes.filter((cls) =>
        cls.lecturers.some((lec) => normalizeName(lec) === lecturerName)
      );
    } else if (currentUser.role === "student") {
      classes = classes.filter(
        (cls) =>
          cls.classTypes.includes(currentUser.classType!) &&
          !cls.lecturers.some((lecturerName) =>
            suspendedLecturerNames.has(lecturerName)
          )
      );
    }
  ...
  const userNotifications = useMemo(() => {
    if (!currentUser || currentUser.role === "administrator") return [];
    const registrationTime = currentUser.registrationDate
      ? new Date(currentUser.registrationDate).getTime()
      : 0;
    const timeFilteredNotifications = notifications.filter(
      (n) => new Date(n.date).getTime() >= registrationTime
    );
    const visibleClassIds = new Set(allUserVisibleClasses.map((c) => c.id));

    let relevantNotifications = timeFilteredNotifications.filter(
      (n) =>
    ...
    if (currentUser.role === "lecturer") {
      relevantNotifications = relevantNotifications.filter(
        (notif) =>
          !notif.message.includes("telah dibatalkan") &&
          !notif.message.includes("mengalami perubahan")
      );
    }
  ...
  useEffect(() => {
    const timer = setInterval(
      () => setRealtimeDate((prevDate) => new Date(prevDate.getTime() + 1000)),
      1000
    );
  ...
  useEffect(() => {
    ...
    const processClassStatusUpdates = async () => {
      ...
      for (const cls of allClasses) {
        if (
          cls.status === ClassStatus.Selesai ||
        ...
        if (cls.status !== newStatus) {
          ...
          let message = "";
          if (newStatus === ClassStatus.Aktif)
            message = `Kelas ${cls.name} telah dimulai.`;
          else if (newStatus === ClassStatus.Selesai)
            message = `Kelas ${cls.name} telah berakhir.`;
          if (message) {
            const newNotif = {
    ...
    if (reminder !== null) {
      userClasses.forEach((cls) => {
        if (
          cls.status === ClassStatus.Belum ||
          cls.status === ClassStatus.Segera
        ) {
          const reminderTime = new Date(cls.start.getTime() - reminder * 60000);
          if (
            currentDate.getTime() >= reminderTime.getTime() &&
            currentDate.getTime() < reminderTime.getTime() + 1000
          ) {
  ...
  const handleSetView = (newView: View) => {
    ...
    if (newView === View.CALENDAR && calendarDisplayDate === null)
      setCalendarDisplayDate(new Date(realtimeDate));
    if (view === View.DASHBOARD && newView !== View.DASHBOARD) {
      setIsSearchActive(false);
      setSearchQuery("");
    }
    if (view === View.NOTIFICATIONS && newView !== View.NOTIFICATIONS)
      markAllNotificationsAsRead();
    setView(newView);
  };

  const addClass = async (
    classData: FullNewClassData
  ): Promise<string | null> => {
    if (!currentUser || currentUser.role !== "lecturer")
      return "Hanya dosen yang bisa menambah kelas.";

    const startDateTime = new Date(classData.start);
    if (startDateTime < realtimeDate)
      return `Tidak dapat menjadwalkan kelas "${classData.name}" pada waktu yang sudah berlalu.`;
    const timeDifference = startDateTime.getTime() - realtimeDate.getTime();
    if (timeDifference < 30 * 60 * 1000)
      return `Kelas "${classData.name}" harus dijadwalkan minimal 30 menit dari sekarang.`;

    const conflictError = checkClassConflict(
      classData,
      allClasses,
      currentUser.name
    );
    ...
    const newClass: Omit<ClassInstance, "$id"> = {
      ...
      status: getClassStatus(
        { ...classData, createdAt: new Date(), status: ClassStatus.Belum },
        realtimeDate
      ),
    };
    ...
    try {
      ...
      setAllClasses((prev) =>
        [...prev, { ...newClass, $id: createdDoc.$id }].sort(
  ...
  const sidebarButtonJustifyClass = isDesktopSidebarCollapsed
    ? "justify-center"
    : "justify-start";
  ...
  const buttonTextClass = `text-base whitespace-nowrap overflow-hidden transition-all duration-150 ${
    isDesktopSidebarCollapsed ? "w-0 opacity-0" : "opacity-100 ml-4"
  }`;
  const profileTextClass = `flex flex-col items-start text-left whitespace-nowrap overflow-hidden transition-all duration-150 ${
    isDesktopSidebarCollapsed ? "w-0 opacity-0" : "opacity-100 ml-3"
  }`;
  const footerButtonClass =
    "flex flex-col items-center justify-center h-full w-full rounded-lg transition-colors duration-200";
  const activeFooterButtonClass = "bg-primary text-header-text";
  const inactiveFooterButtonClass = "text-text";

  const renderView = () => {
    /* ... same as before ... */
  }; // Placeholder, this is too large to repeat
